...
- Features: Automated test generation, advanced mocking, coverage analysis, performance benchmarking, integration testing, custom matchers, multiple reporters
- Supports cross-server testing and compatibility validation

**MCP Test Runner** (privsim/mcp-test-runner):
- Unified interface for executing multiple testing frameworks (Pytest, Jest, Go, Rust, Flutter, Bats)
...
- Intentionally extensible: Allows tags, categorization, curation data via subregistries
- OpenAPI-compatible: Registry itself published as OpenAPI spec for subregistry implementation
...
- `json-schema-to-zod`: Convert JSON Schema → Zod (for TypeScript-first projects)
...
describe('MCP Tools', () => {
  ...
  it('validates input against schema', async () => {
    ...
    expect(() => 
      server.callTool('test_tool', { input: 123 })
    ).toThrow();
  });
});
```
