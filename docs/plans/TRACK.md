...
- `claude/*` branch prefix is not in `scripts/validate-branch-name.sh`'s allow
  list; pre-commit on this branch will reject. Decide whether to extend the
  validator or only commit feat work from a branch matching the existing
  pattern.
