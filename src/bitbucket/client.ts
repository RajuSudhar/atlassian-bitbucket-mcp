  ...
  async request<T>(endpoint: string, options: RequestOptions = {}): Promise<T> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        log('debug', 'bb res', { operation: 'api_response', endpoint, method, statusCode: response.status, durationMs });
        ...
        if (!RETRYABLE_STATUS.has(response.status)) {
          ...
          throw new BitbucketApiError(response.status, endpoint, method, body || response.statusText);
        }
        ...
        if (response.status === 429) {
          ...
          if (retryAfter) {
            log('warn', 'rate limited', { operation: 'rate_limit', endpoint, retryAfterMs: retryAfter });
        ...
        log('warn', 'retrying request', { operation: 'retry', endpoint, attempt: attempt + 1, statusCode: response.status });
      } catch (err) {
        if (err instanceof BitbucketApiError) throw err;
        if (err instanceof TimeoutError) {
          if (attempt === maxRetries) throw err;
          log('warn', 'retrying after timeout', { operation: 'retry', endpoint, attempt: attempt + 1 });
          continue;
        }
        if (attempt === maxRetries) {
          throw new NetworkError(endpoint, err);
        }
        log('warn', 'retrying after network error', { operation: 'retry', endpoint, attempt: attempt + 1 });
      }
    }
