...
# 0b. Regenerate types if any openapi/*.yaml changed; fail on drift
if git diff --cached --name-only | grep -qE "^openapi/.*\.ya?ml$"; then
  echo "[0b/8] Checking openapi -> types drift (openapi/*.yaml changed)..."
  ./scripts/check-types-drift.sh
  if [ $? -ne 0 ]; then
    echo "ERROR: openapi -> types drift detected!"
    exit 1
  fi
