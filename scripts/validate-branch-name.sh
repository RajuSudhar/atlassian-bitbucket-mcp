...
# Harness-managed branches (Claude Code on the web) — opaque session suffix is
# generated by the harness and may include mixed case in any position.
# Always allowed.
if echo "$current_branch" | grep -qE "^claude/[A-Za-z][a-zA-Z0-9-]+$"; then
  exit 0
fi
