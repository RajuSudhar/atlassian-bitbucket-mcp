...
Build the in-memory MCP test harness, the Bitbucket API mock layer using MSW, and the schema validation engine — the three pillars required for the declarative test runner in Phase 5.
...
export class TestHarness {
  ...
  private toolHandlers: Map<string, (args: Record<string, unknown>) => Promise<CallToolResult>> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    const loadResult = loadToolsFromDirectory(
      this.options.definitionsDir,
  ...
  setHandler(
    ...
    if (!this.toolDefinitions.has(toolName)) {
      throw new Error(`Unknown tool: ${toolName}. Available: ${[...this.toolDefinitions.keys()].join(', ')}`);
    }
...
export function assertProperty(
  ...
  for (const [op, expected] of Object.entries(assertion)) {
    switch (op) {
      case 'eq':
        if (value !== expected) {
          return { pass: false, message: `${path}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(value)}` };
        }
        break;
      case 'neq':
        if (value === expected) {
          return { pass: false, message: `${path}: expected not equal to ${JSON.stringify(expected)}` };
        }
        break;
      case 'gt':
      ...
      case 'contains':
        if (typeof value !== 'string' || !value.includes(expected as string)) {
          return { pass: false, message: `${path}: expected to contain "${expected}", got "${value}"` };
        }
        break;
      case 'matches':
        if (typeof value !== 'string' || !new RegExp(expected as string).test(value)) {
          return { pass: false, message: `${path}: expected to match /${expected}/, got "${value}"` };
        }
        break;
      case 'length':
