...
Define the core TypeScript types for the SSOT system (`ToolDefinition`, `TestCase`, `TestSuite`, etc.) in `src/lib/types.ts`, build a YAML loader that parses and validates tool definition files against a meta-schema, and establish the SDK barrel export.
...
**CRITICAL**: The `JSONSchema7` type may need to be defined locally if `json-schema-to-zod` does not export it. In that case, define a compatible type:
...
The loader validates every YAML file against this JSON Schema. This ensures structural correctness before any runtime usage.
...
export function loadToolsFromDirectory(
...
**IMPORTANT**: All exports use `.js` extensions because this is an ESM project. TypeScript resolves `.js` imports to `.ts` files at compile time.
