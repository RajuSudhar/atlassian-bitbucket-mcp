...
Build a complete YAML-driven Single Source of Truth (SSOT) system for defining,
validating, code-generating, and testing all 20 MCP tools in the Atlassian
Bitbucket MCP server. The system produces TypeScript types, Zod validators, and
a declarative test runner from YAML tool definitions, and is structured as an
extractable SDK from day one.
...
4. All 20 tools have declarative YAML test cases that validate input schemas,
   output schemas, and mock API interactions
5. The `src/lib/` directory is a self-contained, extractable SDK with zero
   imports from `src/` outside `lib/`
